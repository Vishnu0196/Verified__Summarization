...
#include <queue>
#include <random>
#include "/Users/jalend15/Downloads/boost_1_77_0/boost/math/distributions/beta.hpp"
#include<cmath>
#include <ctime>
#include <sys/types.h>
#include <sys/stat.h>
...
// random_device rd;
mt19937 gen(seed);
boost::math::beta_distribution<> mybeta(2,5);
...
void compute_beta_distribution()
{
	...
	for(int k = 0; k < K; ++k)
	{
		for(int t = 1; t < RESOLUTION; ++t)
		{
			...
			pkt[k][t] = log(pow(ts, alpha_kt[k]-1)) + log(pow(1-ts, beta_kt[k]-1)) - log(mybeta.find_beta(alpha_kt[k], beta_kt[k]));
		}
	}
	...
	for(int s = 0; s < T; ++s)
	{
		for(int t = 1; t < RESOLUTION; ++t)
		{
			...
			pst[s][t] = log(pow(ts, alpha_st[s]-1)) + log(pow(1-ts, beta_st[s]-1)) - log(mybeta.find_beta(alpha_st[s], beta_st[s])); //tr1::
		}
	}
...
int main(int argc, char const *argv[])
{
	...
	string basepath = "/Users/jalend15/Downloads/Verified-Summarization-master/CTP/";
	string destpath = "/Users/jalend15/Downloads/Verified-Summarization-master/CTP/output_"+to_string(K)+"_"+to_string(T)+"_"+to_string(numIter)+"/";
